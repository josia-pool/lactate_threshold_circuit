...
class Bassystem_sRNA(ODEModel):
    def __init__(self):
        ...
        #sRNA system parameters
        k_b_srna = 30 #Constitutive production of sRNA - Unit: mM/hr
        k_d_srna = 2 #sRNA degradation - Unit: 1/hr

        k_d_sgrna2 = 1.389 #sgRNA2 degradation - Unit: 1/hr

        k_f_cascomplex2_fw = 5 #Formation rate of dCas-sgRNA2 complex - Unit: mM^-1/hr
        k_f_cascomplex2_rv = 0.855 #Dissociation rate of CasComplex2 - Unit: 1/hr

        k_d_gfpmrna2 = 10 #sRNA mediated degradation of GFP_mRNA - Unit: 1/hr
        kd_srna = 0.05 #Dissociation/ Michealis menten constant of sRNA repression - Unit: mM/dimensionless? Cancels out either way

        #Pack the parameters
        self.parameters_standard = [k_b_dcas,k_op_succ, k_b_p3, k_b_sgrna,k_d_ascascomplex,k_d_ascomplex,k_d_asrna,k_d_cascomplex,k_d_dcas,k_d_dcasmrna,k_d_gfp,k_d_gfpmrna,k_d_lldr,k_d_lldrcomplex,k_d_lldrmrna,k_d_sgrna,k_f_ascascomplex1_fw,k_f_ascascomplex1_rv,k_f_ascascomplex2_fw,k_f_ascascomplex2_rv,k_f_ascomplex_fw,k_f_ascomplex_rv,k_f_cascomplex_fw,k_f_cascomplex_rv,k_f_lldrcomplex,alpaga_basal,alpaga_act1,alpaga_act2,alpaga_k_act1,alpaga_k_act2,alpaga_k3,k_p_gfpmrna,k_pt_dcas,k_pt_gfp,k_pt_lldr,kd_dcas,n_alpaga,n_cascomplex,copy_nr,mumax,tc,cap,sigma, k_b_srna,k_d_srna, k_d_sgrna2,k_f_cascomplex2_fw, k_f_cascomplex2_rv, k_d_gfpmrna2, kd_srna, gamma, scale]

        self.params = self.parameters_standard
        
        self.param_labels =         ['k_b_dcas',        'k_op_succ',        'k_b_p3',    'k_b_sgrna','k_d_ascascomplex','k_d_ascomplex','k_d_asrna','k_d_cascomplex','k_d_dcas','k_d_dcasmrna','k_d_gfp','k_d_gfpmrna','k_d_lldr','k_d_lldrcomplex','k_d_lldrmrna','k_d_sgrna',
        'k_f_ascascomplex1_fw','k_f_ascascomplex1_rv','k_f_ascascomplex2_fw','k_f_ascascomplex2_rv','k_f_ascomplex_fw','k_f_ascomplex_rv','k_f_cascomplex_fw','k_f_cascomplex_rv','k_f_lldrcomplex','alpaga_basal','alpaga_act1','alpaga_act2','alpaga_k_act1','alpaga_k_act2','alpaga_k3','k_p_gfpmrna','k_pt_dcas','k_pt_gfp','k_pt_lldr','kd_dcas','n_alpaga','n_cascomplex','copy_nr','mumax','tc','cap','sigma', 'k_b_srna','k_d_srna', 'k_d_sgrna2','k_f_cascomplex2_fw', 'k_f_cascomplex2_rv', 'k_d_gfpmrna2', 'kd_srna','scale']
        self.plot_param_labels =    ['$k\_b_{dcas}$',   '$k\_op_{succ}$', '$k\_b_{p3}$', '$k\_b_{sgrna}$','$k\_d_{ascascomplex}$','$k\_d_{ascomplex}$','$k\_d_{asrna}$','$k\_d_{cascomplex}$','$k\_d_{dcas}$','$k\_d_{dcasmrna}$','$k\_d_{gfp}$','$k\_d_{gfpmrna}$','$k\_d_{lldr}$','$k\_d_{lldrcomplex}$','$k\_d_{lldrmrna}$','$k\_d_{sgrna}$',
        '$k\_f_{3_{fw}}$'   ,'$k\_f_{3_{rv}}$','$k\_f_{4_{fw}}$','$k\_f_{4_{rv}}$','$k\_f_{2_{fw}}$','$k\_f_{2_{rv}}$','$k\_f_{1_{fw}}$','$k\_f_{1_{rv}}$','$k\_f_{lldrcomplex}$','$k\_b_{alp}$','$alp_{act1}$','$alp_{act2}$','$alp\_k_{act1}$','$alp\_k_{act2}$','$alp_{k3}$','$k\_p_{gfpmrna}$','$k\_pt_{dcas}$','$k\_pt_{gfp}$','$k\_pt_{lldr}$','$kd_{dcas}$','$n_{alpaga}$','$n_{cascomplex}$','$copy\_nr$','$\mu_{max}$','$t_c$','$cap$','$\sigma$', '$k\_b_{sRNA}', '$k\_d_{sRNA}$', '$k\_d_{sgRNA2}$', '$k\_f_{cascomplex2_{fw}}$', '$k\_f_{cascomplex2_{rv}}$', '$k\_d_{gfpmrna2}$', '$kd_{sRNA}$','$gamma$','$scale$']
        ...
        Cx = 0.05
        #Steady state approximation - Assuming no lactate and no CasComplex at the start.
        #Lldr_mRna = k_b_lldr/k_d_lldrmrna
        #lldr = (k_pt_lldr*k_b_lldr)/(k_d_lldrmrna*k_d_lldr) - Doesnt fix the sudden spike in lldrcomplex
        ...
        self.initial_conditions = [dCas_mRNA, dCas, sgRNA, CasComplex, asRNA, asComplex, asCasComplex, Lactate, Lldr_mRna, lldr, lldr_complex, sRNA, sgRNA2, CasComplex2, GFP_mRNA, GFP, Cx]
        ...
        self.labels = ['dCas_mRNA', 'dCas', 'sgRNA', 'casComplex', 'asRNA', 'asComplex', 'asCasComplex', 'Lactate', 'Lldr_mRna', 'lldr', 'lldr_complex', 'sRNA', 'sgRNA2', 'GFP_mRna', 'GFP', 'Biomass']
    ...
    def deriv(self, y, t, p):
        ...
        dCas_mRNA, dCas, sgRNA, CasComplex, asRNA, asComplex, asCasComplex, Lactate, Lldr_mRna, lldr, lldr_complex, sRNA, sgRNA2, CasComplex2, GFP_mRNA, GFP, Cx= y
        #Unpack the parameter vector
        k_b_dcas,k_op_succ, k_b_p3, k_b_sgrna,k_d_ascascomplex,k_d_ascomplex,k_d_asrna,k_d_cascomplex,k_d_dcas,k_d_dcasmrna,k_d_gfp,k_d_gfpmrna,k_d_lldr,k_d_lldrcomplex,k_d_lldrmrna,k_d_sgrna,k_f_ascascomplex1_fw,k_f_ascascomplex1_rv,k_f_ascascomplex2_fw,k_f_ascascomplex2_rv,k_f_ascomplex_fw,k_f_ascomplex_rv,k_f_cascomplex_fw,k_f_cascomplex_rv,k_f_lldrcomplex,alpaga_basal,alpaga_act1,alpaga_act2,alpaga_k_act1,alpaga_k_act2,alpaga_k3,k_p_gfpmrna,k_pt_dcas,k_pt_gfp,k_pt_lldr,kd_dcas,n_alpaga,n_cascomplex,copy_nr,mumax,tc,cap,sigma, k_b_srna,k_d_srna, k_d_sgrna2,k_f_cascomplex2_fw, k_f_cascomplex2_rv, k_d_gfpmrna2, kd_srna, gamma, scale = p
        ...
        d_sRNA_dt = mu/mumax*(k_b_srna*copy_nr*(kd_dcas**n_cascomplex/(kd_dcas**n_cascomplex + CasComplex2**n_cascomplex))) - sRNA*(k_d_srna+mu)
        d_sgRNA2_dt = mu/mumax*(top_a/bottom_a*copy_nr) - sgRNA2*(k_d_sgrna2+mu) + k_f_cascomplex2_rv*CasComplex2 - k_f_cascomplex2_fw*sgRNA2*dCas
        d_CasComplex2_dt = k_f_cascomplex2_fw*sgRNA2*dCas - k_f_cascomplex2_rv*CasComplex2 - CasComplex2*(k_d_cascomplex+mu)
        d_GFP_mRNA_dt   = mu/mumax*(copy_nr*top_a/bottom_a*(kd_dcas**n_cascomplex/(kd_dcas**n_cascomplex + CasComplex**n_cascomplex)))- GFP_mRNA*(k_d_gfpmrna+k_d_gfpmrna2*(sRNA/(kd_srna+sRNA))+mu)
        d_GFP_dt        = mu*k_pt_gfp*GFP_mRNA-GFP*(k_d_gfp+mu)
        d_Cx_dt = Cx*mu

        #Pack the derivatives
        dydt = [d_dCas_mRNA_dt, d_dCas_dt, d_sgRNA_dt, d_CasComplex_dt, d_asRNA_dt, d_asComplex_dt, d_asCasComplex_dt, d_Lactate_dt, d_Lldr_mRna_dt, d_lldr_dt, d_lldr_complex_dt, d_sRNA_dt, d_sgRNA2_dt, d_CasComplex2_dt, d_GFP_mRNA_dt, d_GFP_dt, d_Cx_dt]
        return dydt
    ...
    def simulate_experiment(self, p, init, concentration1, concentration2):
        ...
        """
        #Round off hill coefficients
        p[36] = 1 if p[36] < 0.5 else round(p[36])
        p[37] = 1 if p[37] < 0.5 else round(p[37])
    ...
    def oaat_analysis_mp(self, _num = 9, folder='figures', plot_data = False):
        ...
        #Doesnt work yet
        #standard_hbarchart(labels, ['Factor 10 increase', 'Factor 10 decrease'], [[factor_10_increase],[factor_10_decrease]], np.zeros((len(factor_10_increase),20)), "oaat_standardhbar", x_label= "", y_label= "", _figwidth=16, _figheight=9, _nozeroerror = True)
...
if __name__ == '__main__':
    ...
    #     print(f"{model.param_labels[i]}: {param}")
    ...
    plt.plot(concentrations, responses, label="Simulated", color=colors['Green'])
    plt.plot(ec, ec_response, 'o', label="EC5,50,95", color=colors['Red'])
    plt.axvspan(1.5,3, color=colors['Green'], alpha=0.3, label='Healthy')
    plt.axvspan(10,25, color=colors['Red'], alpha=0.3, label='Cancer')
    plt.axvline(1.5,ymin =0, ymax=1, color=colors['Green'], linestyle=dashline, linewidth=2)
    plt.axvline(3,ymin =0, ymax=1, color=colors['Green'], linestyle=dashline, linewidth=2)
    ...
    plt.ylabel("Fluorescence/OD600 (a.u.)")
    plt.legend()
    plt.savefig(f"figures/dose_response_curve_sRNA.svg", dpi= 300)
    plt.close()

    fig, ax = plt.subplots(figsize=(8, 4.5), tight_layout = True)
    plt.plot(concentrations, responses/max(responses), label="Simulated", color=colors['Green'])
    plt.plot(ec, ec_response/max(responses), 'o', label="EC5,50,95", color=colors['Red'])
    plt.axvspan(1.5,3, color=colors['Green'], alpha=0.3, label='Healthy')
    plt.axvspan(10,25, color=colors['Red'], alpha=0.3, label='Cancer')
    plt.axvline(1.5,ymin =0, ymax=1, color=colors['Green'], linestyle=dashline, linewidth=2)
    plt.axvline(3,ymin =0, ymax=1, color=colors['Green'], linestyle=dashline, linewidth=2)
    ...
    plt.legend()
    plt.savefig(f"figures/norm_dose_response_curve_sRNA.svg", dpi= 300)
    plt.close()
    ...
    plt.legend()
    plt.savefig('figures/scaled_fluorescence_sRNA.svg', dpi = 200)
    plt.close()
    
    model.oaat_analysis_mp(folder="figures/oaat_analysis_sRNA_system")
