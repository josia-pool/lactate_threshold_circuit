...
class Bassystem_FFN(ODEModel):
    ...
    def deriv(self, y, t, p):
        ...
        d_GFP_mRNA_dt   = mu/mumax*(copy_nr*(top_a/bottom_a)*(kd_dcas**n_cascomplex/(kd_dcas**n_cascomplex + CasComplex**n_cascomplex)))- GFP_mRNA*(k_d_gfpmrna+mu)
...
if __name__ == '__main__':
    ...
    plt.ylabel("Fluorescence/OD600 (a.u.)")
    plt.legend()
    plt.savefig(f"figures/dose_response_curve_GFPalpaga.svg", dpi= 300)
    plt.close()
    ...
    plt.legend()
    plt.savefig(f"figures/norm_dose_response_curve_GFPalpaga.svg", dpi= 300)
    plt.close()
    ...
    plt.legend()
    plt.savefig('figures/scaled_fluorescence_gfpalpaga.svg', dpi = 200)
    plt.close()
    
    model.oaat_analysis_mp(folder="figures/oaat_analysis_fullsystem_FFL")
